...
def register_mcp_instance(mcp_instance: Any) -> None:
    """Register both tools with an MCP instance when available."""
    global gov_policy_summary
    if mcp_instance is None:
        return
    # register search tool and download tool
    try:
        mcp_instance.tool(govinfo_search_granules)
    except Exception:
        # best-effort registration; don't fail import
        pass
    try:
...
async def federal_register_get_document_summary(document_id: str, fmt: str = "htm") -> Dict[str, Any]:
    """Fetch a Federal Register document summary and optionally its HTML/text content.

    Returns similar envelope as GovInfo summary tool for compatibility.
    """
    start = time.time()
    if not document_id:
...
def register_mcp_instance(mcp_instance: Any) -> None:
    """Register both GovInfo and Federal Register summary tools with MCP."""
    if mcp_instance is None:
        return
    try:
        mcp_instance.tool(govinfo_search_granules)
